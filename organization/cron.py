    ...
    for branch in branches:
        ...
        for terminal in terminals:
            ...
            if (terminal.dayclose or no_bill) == False:
                ...
                terminal_to_close_their_day.append(terminal)
                break
            else:
                daycloseForallterminal=True
        if daycloseForallterminal == True:
            branch_queryset_approve.append(branch)
        branch_approve.append(daycloseForallterminal)
    print(f"These are the branches: {branch_queryset_approve}")
        # branch_approve.append(daycloseForallterminal)
    
    # daycloseForallBranch = False
    # for approve in branch_approve:
    #     if approve:
    #         daycloseForallBranch = True
    #     else:
    #         daycloseForallBranch = False
    #         break
    # return daycloseForallBranch

    return branch_queryset_approve
...
def end_day():
    ...
    auto_end_day= org.auto_end_day
    # branches = Branch.objects.all()

    if auto_end_day:

        branches = check_day_close()

        print(f"These are the branches {branches}")
        ...
        if branches != []:
                ...
                for terminal_id in terminal_ids:
                    ...
                    if (
                        ...
                        try:
                            ...
                            terminal = Terminal.objects.get(branch=branch, terminal_no=terminal_id, is_deleted=False, status=True)
            ...
            print("dayclose was not done for some branch")
    else:
        print("auto_end_day was not enabled")
