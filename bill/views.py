...
from .models import Bill  # Import your Bill model

from datetime import datetime, timedelta

class BillList(BillMixin, ListView):
    template_name = "bill/bill_list.html"
    model = Bill  # Use your Bill model

    # def get_queryset(self):
    #     queryset = super().get_queryset()
        
    #     # print(queryset)
    #     # Check if the user has pressed the search button by looking for 'search' in the query parameters
    ...
    #     if from_date and to_date:
    #             # Assuming 'transaction_date' is the date field in your Bill model
    #         queryset = queryset.filter(created_at__range=[from_date, to_date])
    #         print(queryset)
...
class CashDropExportExcel(View):
    def get(self, request, *args, **kwargs):
        date_filter = request.GET.get('datetime')
        print(date_filter)
        today_date = datetime.today().date()
        today_date_str = today_date.strftime("%Y-%m-%d")

        response = HttpResponse(content_type="application/ms-excel")
        response["Content-Disposition"] = 'attachment; filename="cashdrop_data.xls"'

        wb = xlwt.Workbook(encoding="utf-8")
        ws = wb.add_sheet("Daily Sales Data")
        ...
        ws.write_merge(0, 0, 0, len(columns) - 1, "Daily Sales Data", font_style_organization)
        ws.row(0).height = 256 * 2

        new_columns = ["branch"] + columns[1:]
        for col_num in range(len(columns)):
            ws.write(row_num, col_num, new_columns[col_num], font_style_bold)

        rows = CashDrop.objects.filter(datetime__startswith=today_date_str).values_list(*columns)
        if date_filter:
            rows = CashDrop.objects.filter(datetime__startswith=date_filter).values_list(*columns)

        
        for row in rows:
            row_num += 1
            for col_num in range(len(row)):
                if columns[col_num] == "datetime":
                    ws.write(
                        row_num,
                        col_num,
                        row[col_num],
